...
inline pixel_type_w SubsampleTendency(pixel_type_w pavg, pixel_type_w avg,
                                      pixel_type_w navg) {
  return ((3 * avg + pavg + 1) >> 2) - ((3 * avg + navg + 2) >> 2);
}

void InvHSqueeze(Image &input, int c, int rc, bool subsample_mode,
                 ThreadPool *pool) {
  ...
  if (!subsample_mode) {
    ...
    RunOnPool(
        ...
        [&](const int task, const int thread) {
          ...
          for (size_t x = 0; x < chin_residual.w; x++) {
            pixel_type_w diff_minus_tendency = p_residual[x];
            pixel_type_w avg = p_avg[x];
            pixel_type_w prev_avg = x == 0 ? p_avg[0] : p_avg[x - 1];
            pixel_type_w next_avg =
                (x + 1 < chin.w ? next_avg = p_avg[x + 1] : avg);
            pixel_type_w tendency = SubsampleTendency(prev_avg, avg, next_avg);
            pixel_type_w diff = diff_minus_tendency + tendency;
            pixel_type_w A =
                ((avg * 2) + diff + (diff > 0 ? -(diff & 1) : (diff & 1))) >> 1;
            p_out[x << 1] = ClampToRange<pixel_type>(A);
        ...
        "InvHorizontalSubsampleSqueeze");
  }
  input.channel[c] = std::move(chout);
}

void FwdHSqueeze(Image &input, int c, int rc, bool subsample_mode) {
  const Channel &chin = input.channel[c];
  ...
  if (!subsample_mode) {
    ...
    for (size_t y = 0; y < chout.h; y++) {
      ...
      for (size_t x = 0; x < chout_residual.w; x++) {
        ...
        pixel_type prev_avg = x == 0 ? p_out[0] : p_out[x - 1];
        pixel_type next_avg = avg;
        if (x + 1 < chout_residual.w) {
          next_avg = (p_in[x * 2 + 2] + p_in[x * 2 + 3] +
                      (p_in[x * 2 + 2] > p_in[x * 2 + 3])) >>
...
void InvVSqueeze(Image &input, int c, int rc, bool subsample_mode,
                 ThreadPool *pool) {
  const Channel &chin = input.channel[c];
  const Channel &chin_residual = input.channel[rc];
  ...
  if (!subsample_mode) {
    RunOnPool(
        ...
        [&](const int task, const int thread) {
          ...
          const size_t x1 =
              std::min((size_t)(task + 1) * kColsPerThread, chin.w);
          for (size_t y = 0; y < chin_residual.h; y++) {
            ...
            for (size_t x = x0; x < x1; x++) {
              ...
              pixel_type_w diff = diff_minus_tendency + tendency;
              pixel_type_w out =
                  ((avg * 2) + diff + (diff > 0 ? -(diff & 1) : (diff & 1))) >>
                  1;
        ...
        [&](const int task, const int thread) {
          ...
          for (size_t y = 0; y < chin_residual.h; y++) {
            ...
            for (size_t x = x0; x < x1; x++) {
              ...
              pixel_type_w next_avg = avg;
              if (y + 1 < chin.h) next_avg = p_avg[x + onerow_in];
              pixel_type_w prev_avg =
                  y == 0 ? p_avg[x]
                         : p_avg[static_cast<ssize_t>(x) - onerow_in];
              pixel_type_w tendency =
                  SubsampleTendency(prev_avg, avg, next_avg);
              pixel_type_w diff = diff_minus_tendency + tendency;
              pixel_type_w out =
                  ((avg * 2) + diff + (diff > 0 ? -(diff & 1) : (diff & 1))) >>
                  1;
...
void FwdVSqueeze(Image &input, int c, int rc, bool subsample_mode) {
  const Channel &chin = input.channel[c];
  ...
  if (!subsample_mode) {
    ...
    for (size_t y = 0; y < chout_residual.h; y++) {
      ...
      for (size_t x = 0; x < chout.w; x++) {
        ...
        pixel_type_w prev_avg =
            y == 0 ? p_out[x] : p_out[static_cast<ssize_t>(x) - onerow_in];
        pixel_type_w tendency = SubsampleTendency(prev_avg, avg, next_avg);
...
Status InvSqueeze(Image &input, std::vector<SqueezeParams> parameters,
  ...
  for (int i = parameters.size() - 1; i >= 0; i--) {
    bool horizontal = parameters[i].horizontal;
    bool in_place = parameters[i].in_place;
    bool subsample_mode = parameters[i].subsample_mode;
    uint32_t beginc = parameters[i].begin_c;
    uint32_t endc = parameters[i].begin_c + parameters[i].num_c - 1;
    uint32_t offset;
    if (in_place) {
    ...
    for (uint32_t c = beginc; c <= endc; c++) {
      ...
      if (horizontal) {
        InvHSqueeze(input, c, offset + c - beginc, subsample_mode, pool);
      } else {
        InvVSqueeze(input, c, offset + c - beginc, subsample_mode, pool);
      }
    }
...
Status FwdSqueeze(Image &input, std::vector<SqueezeParams> parameters,
  ...
  for (size_t i = 0; i < parameters.size(); i++) {
    ...
    for (uint32_t c = beginc; c <= endc; c++) {
      if (horizontal) {
        FwdHSqueeze(input, c, offset + c - beginc, subsample_mode);
      } else {
        FwdVSqueeze(input, c, offset + c - beginc, subsample_mode);
      }
    }
  }
  return true;
