...
class BitReader {
 public:
  ...
  HWY_ATTR explicit BitReader(const ArrayLike& bytes)
      : buf_(0),
        bits_in_buf_(0),
  ...
  HWY_ATTR JXL_INLINE void Refill() {
  ...
  HWY_INLINE void Consume(size_t num_bits) {
    JXL_DASSERT(!close_called_);
  ...
  HWY_ATTR JXL_INLINE uint64_t ReadBits(size_t nbits) {
    JXL_DASSERT(!close_called_);
    Refill();
  ...
  template <size_t N>
  HWY_ATTR JXL_INLINE uint64_t ReadFixedBits() {
    JXL_DASSERT(!close_called_);
    Refill();
  ...
  HWY_ATTR void SkipBits(size_t skip) {
    JXL_DASSERT(!close_called_);
  ...
  HWY_ATTR Status JumpToByteBoundary() {
