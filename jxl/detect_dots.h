...
#ifndef JXL_DETECT_DOTS_H_
#define JXL_DETECT_DOTS_H_
...
struct GaussianDetectParams {
  double t_high;  // at least one pixel must have larger energy than t_high
  double t_low;   // all pixels must have a larger energy than tLow
  uint32_t maxWinSize;  // discard dots larger than this containing window
  double maxL2Loss;
  double maxCustomLoss;
  double minIntensity;     // If the intensity is too low, discard it
  double maxDistMeanMode;  // The mean and the mode must be close
  size_t maxNegPixels;     // Maximum number of negative pixel
  size_t minScore;
