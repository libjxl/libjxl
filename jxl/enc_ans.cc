...
#include "c/enc/huffman_encode.h"
...
size_t BuildAndStoreANSEncodingData(
  ...
  if (use_prefix_code) {
    ...
    std::vector<uint8_t> storage(1024 + alphabet_size);
    brunsli::Storage st(storage.data(), storage.size());
    brunsli::BuildAndStoreHuffmanTree(histo.data(), alphabet_size,
                                      depths.data(), bits.data(), &st);
    for (size_t i = 0; i < alphabet_size; i++) {
