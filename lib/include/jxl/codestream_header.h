...
typedef struct {
   ...
   * the decoder to the original color profile, one of the @ref JxlDecoderGetColor
   * "JxlDecoderGetColor" * functions needs to be called with @ref JXL_COLOR_PROFILE_TARGET_DATA to get
   ...
   * If and only if this is 1, the @ref JxlColorSpace in the @ref JxlColorEncoding is
   ...
   * can be queried with @ref JxlDecoderGetExtraChannelInfo. The main alpha channel,
   * if it exists, also has its information available in the alpha_bits,
   * alpha_exponent_bits and alpha_premultiplied fields in this @ref JxlBasicInfo.
   */
...
typedef struct {
   ...
   * tick is given by tps_numerator and tps_denominator in @ref JxlAnimationHeader.
   */
  uint32_t duration;
