...
#if !defined(JXL_CMS_INTERFACE_H_) || defined(CUSTOM_GENERATOR)
...
#	else
		ESCAPE(#ifndef JXL_CMS_INTERFACE_H_)
		ESCAPE(#define JXL_CMS_INTERFACE_H_)
...
EXTERN_C(
	...
	StructDef(JxlColorProfile,
		/** The serialized ICC profile. This is guaranteed to be present and valid. */
		NamedInlineStruct(icc,
			Member(const uint8_t*, data)
			Member(size_t, size)
		)
		 ...
		 */
		Member(JxlColorEncoding, color_encoding)
	 ...
	 */
	Delegate(void*, jpegxl_cms_init_func, (void* init_data, size_t num_threads,
	 ...
	 */
	Delegate(float*, jpegxl_cms_get_buffer_func, (void* user_data, size_t thread))
	 ...
	 */
	Delegate(JXL_BOOL, jpegxl_cms_run_func, (void* user_data, size_t thread,
	 ...
	 * data.
	 */
	Delegate(void, jpegxl_cms_destroy_func, (void*))
