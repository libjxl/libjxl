...
TEST(CodecTest, OpaqueAlphaDetectionAndStripping) {
  PackedPixelFile ppf;
  ppf.info.xsize = 2;
  ppf.info.ysize = 2;
  ...
  auto frame_status = PackedFrame::Create(2, 2, format);
  ASSERT_TRUE(frame_status.ok());
  PackedFrame frame = std::move(frame_status).value_();

  // Fill pixels with R, G, B, and A=255
  uint8_t* p = reinterpret_cast<uint8_t*>(frame.color.pixels());
  for (size_t i = 0; i < 4; ++i) {
    p[i * 4 + 0] = static_cast<uint8_t>(10 * i + 1);
    p[i * 4 + 1] = static_cast<uint8_t>(10 * i + 2);
    p[i * 4 + 2] = static_cast<uint8_t>(10 * i + 3);
    p[i * 4 + 3] = 255;
  }
  ppf.frames.push_back(std::move(frame));

  EXPECT_TRUE(ppf.HasAlpha());
  EXPECT_TRUE(ppf.HasOpaqueAlpha());

  // Make one pixel transparent: alpha=254
  p = reinterpret_cast<uint8_t*>(ppf.frames[0].color.pixels());
  p[3] = 254;
  EXPECT_FALSE(ppf.HasOpaqueAlpha());
  ...
  // Drop alpha
  ASSERT_TRUE(ppf.DropAlpha());
  EXPECT_FALSE(ppf.HasAlpha());
  EXPECT_EQ(ppf.info.alpha_bits, 0u);
  EXPECT_EQ(ppf.frames[0].color.format.num_channels, 3u);

  // Check color values are preserved exactly
  const uint8_t* p_dropped =
      reinterpret_cast<const uint8_t*>(ppf.frames[0].color.pixels());
  for (size_t i = 0; i < 4; ++i) {
    EXPECT_EQ(p_dropped[i * 3 + 0], static_cast<uint8_t>(10 * i + 1));
    EXPECT_EQ(p_dropped[i * 3 + 1], static_cast<uint8_t>(10 * i + 2));
    EXPECT_EQ(p_dropped[i * 3 + 2], static_cast<uint8_t>(10 * i + 3));
  }
}

TEST(CodecTest, OpaqueAlpha16BitDetectionAndStripping) {
  PackedPixelFile ppf;
  ppf.info.xsize = 2;
  ppf.info.ysize = 1;
  ppf.info.num_color_channels = 1;
  ppf.info.alpha_bits = 16;
  ppf.info.bits_per_sample = 16;

  JxlPixelFormat format{2, JXL_TYPE_UINT16, JXL_LITTLE_ENDIAN, 0};
  auto frame_status = PackedFrame::Create(2, 1, format);
  ASSERT_TRUE(frame_status.ok());
  PackedFrame frame = std::move(frame_status).value_();

  uint16_t* p = reinterpret_cast<uint16_t*>(frame.color.pixels());
  p[0] = 12345;
  p[1] = 65535;
  p[2] = 23456;
  p[3] = 65535;
  ...
  p = reinterpret_cast<uint16_t*>(ppf.frames[0].color.pixels());
  p[1] = 65534;
  EXPECT_FALSE(ppf.HasOpaqueAlpha());
  p[1] = 65535;
  EXPECT_TRUE(ppf.HasOpaqueAlpha());
  ...
  EXPECT_FALSE(ppf.HasAlpha());
  EXPECT_EQ(ppf.info.alpha_bits, 0u);
  EXPECT_EQ(ppf.frames[0].color.format.num_channels, 1u);

  const uint16_t* p_dropped =
      reinterpret_cast<const uint16_t*>(ppf.frames[0].color.pixels());
  EXPECT_EQ(p_dropped[0], 12345);
  EXPECT_EQ(p_dropped[1], 23456);
}
