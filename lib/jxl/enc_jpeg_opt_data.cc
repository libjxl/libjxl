...
Thresholds JPEGOptData::InitThresh(int axis, uint32_t target_intervals) const {
...
Factorizations JPEGOptData::MaximalFactorizations() const {
    ...
    for (uint32_t c = 0; c < kNumCh; ++c) {
      for (uint32_t y = 0; y < block_grid_h[c]; ++y) {
        for (uint32_t x = 0; x < block_grid_w[c]; ++x) {
          ...
          uint16_t dc0 =
              block_DC_idx[0][(by / ss_y[0]) * block_grid_w[0] + bx / ss_x[0]];
          uint16_t dc1 =
              block_DC_idx[1][(by / ss_y[1]) * block_grid_w[1] + bx / ss_x[1]];
          uint16_t dc2 =
              block_DC_idx[2][(by / ss_y[2]) * block_grid_w[2] + bx / ss_x[2]];
...
// ---------------------------------------------------------------------//
Status JPEGOptData::GenerateRLEStream(ThreadPool* pool) {
  ...
  for (uint32_t c = 0; c < channels; ++c) {
    ...
    for (uint32_t y = 0; y < h; ++y) {
      for (uint32_t x = 0; x < w; ++x) {
        uint32_t b = y * w + x;
        uint32_t b0 =
            (y * ss_y[c] / ss_y[0]) * block_grid_w[0] + (x * ss_x[c] / ss_x[0]);
  ...
  JXL_RETURN_IF_ERROR(RunOnPool(
      ...
      [&](uint32_t chunk, size_t) -> Status {
        ...
        uint32_t idx_end = std::min(idx_start + kChunk,
                                    static_cast<uint32_t>(active_bins.size()));
