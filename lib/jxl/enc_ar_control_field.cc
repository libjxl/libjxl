...
void FindBestArControlField(const Image3F& opsin, PassesEncoderState* enc_state,
  ...
  const auto process_row = [&](size_t gid, int thread) {
    ...
    size_t by1 = std::min((gy + 1) * kGroupDimInBlocks,
    ...
    for (size_t y = y0; y < y1; y++) {
      ...
      for (;
           x + Lanes(df) <= x1 && x + Lanes(df) + bx0 * N - 1 <= opsin.xsize();
           x += Lanes(df)) {
        size_t cx = x + bx0 * N - 2;
        auto sumsqr = Zero(df);
        for (size_t c = 0; c < 3; c++) {
...
#if HWY_ONCE
namespace jxl {
HWY_EXPORT(FindBestArControlField);

void FindBestArControlField(const Image3F& opsin, PassesEncoderState* enc_state,
                            ThreadPool* pool) {
  HWY_DYNAMIC_DISPATCH(FindBestArControlField)(opsin, enc_state, pool);
}
