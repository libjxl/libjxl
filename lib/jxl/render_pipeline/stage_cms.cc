 ...
 public:
  ...
  bool IsNeeded() const {
    ...
    const size_t channels_dst =
        output_encoding_info_.color_encoding.Channels();
    const bool not_mixing_color_and_grey = (channels_src == channels_dst ||
                                      (channels_src == 4 && channels_dst == 3));
    const bool output_is_xyb = output_encoding_info_.color_encoding.GetColorSpace() == ColorSpace::kXYB;
