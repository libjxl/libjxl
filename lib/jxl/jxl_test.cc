...
JXL_SLOW_TEST(JxlTest, RoundtripLossless8LightningGradient) {
  ...
  PackedPixelFile ppf_out;
  // Lacks comparison because different SIMD will cause different compression.
  EXPECT_SLIGHTLY_BELOW(
...
JXL_SLOW_TEST(JxlTest, RoundtripLossless8Falcon) {
  ...
  PackedPixelFile ppf_out;
  EXPECT_EQ(Roundtrip(t.ppf(), cparams, dparams, pool.get(), &ppf_out), 230748);
  EXPECT_EQ(ComputeDistance2(t.ppf(), ppf_out), 0.0);
}
...
TEST(JxlTest, RoundtripLossless8Alpha) {
  ...
  PackedPixelFile ppf_out;
  EXPECT_EQ(Roundtrip(t.ppf(), cparams, dparams, pool, &ppf_out), 251447);
  EXPECT_EQ(ComputeDistance2(t.ppf(), ppf_out), 0.0);
...
TEST(JxlTest, RoundtripLossless8Gray) {
  ...
  PackedPixelFile ppf_out;
  EXPECT_EQ(Roundtrip(t.ppf(), cparams, dparams, pool, &ppf_out), 92164);
  EXPECT_EQ(ComputeDistance2(t.ppf(), ppf_out), 0.0);
