...
HWY_INLINE Vec0<T> ShiftLeft(const Vec0<T> v) {
  ...
  return Vec0<T>(static_cast<hwy::MakeUnsigned<T>>(v.raw) << kBits);
}
...
HWY_INLINE Vec0<T> operator<<(const Vec0<T> v, const Vec0<T> bits) {
  return Vec0<T>(static_cast<hwy::MakeUnsigned<T>>(v.raw) << bits.raw);
}
template <typename T>
HWY_INLINE Vec0<T> operator>>(const Vec0<T> v, const Vec0<T> bits) {
...
HWY_INLINE Vec0<float> And(const Vec0<float> a, const Vec0<float> b) {
  return BitwiseOp<int32_t>()(a, b, [](int32_t i, int32_t j) { return i & j; });
}
HWY_INLINE Vec0<double> And(const Vec0<double> a, const Vec0<double> b) {
...
HWY_INLINE Vec0<float> Or(const Vec0<float> a, const Vec0<float> b) {
  return BitwiseOp<int32_t>()(a, b, [](int32_t i, int32_t j) { return i | j; });
}
HWY_INLINE Vec0<double> Or(const Vec0<double> a, const Vec0<double> b) {
  return BitwiseOp<int64_t>()(a, b, [](int64_t i, int64_t j) { return i | j; });
}
...
HWY_INLINE Vec0<float> Xor(const Vec0<float> a, const Vec0<float> b) {
  return BitwiseOp<int32_t>()(a, b, [](int32_t i, int32_t j) { return i ^ j; });
}
HWY_INLINE Vec0<double> Xor(const Vec0<double> a, const Vec0<double> b) {
  return BitwiseOp<int64_t>()(a, b, [](int64_t i, int64_t j) { return i ^ j; });
}
...
HWY_INLINE Vec0<T> Abs(const Vec0<T> a) {
  const T i = a.raw;
  return (i >= 0 || i == hwy::LimitsMin<T>()) ? a : Vec0<T>(-i);
}
HWY_INLINE Vec0<float> Abs(const Vec0<float> a) {
  return Vec0<float>(std::abs(a.raw));
}
HWY_INLINE Vec0<double> Abs(const Vec0<double> a) {
  return Vec0<double>(std::abs(a.raw));
}
...
HWY_INLINE Vec0<float> Sqrt(const Vec0<float> v) {
  return Vec0<float>(std::sqrt(v.raw));
}
HWY_INLINE Vec0<double> Sqrt(const Vec0<double> v) {
  return Vec0<double>(std::sqrt(v.raw));
}
...
HWY_INLINE Vec0<float> Round(const Vec0<float> v) {
  ...
  if (rounded == 0) return Vec0<float>(v.raw < 0.0f ? -0.0f : 0.0f);
  return Vec0<float>(rounded);
}
HWY_INLINE Vec0<double> Round(const Vec0<double> v) {
  ...
  if (rounded == 0) return Vec0<double>(v.raw < 0.0 ? -0.0 : 0.0);
  return Vec0<double>(std::copysign(rounded, v.raw));
}

HWY_INLINE Vec0<float> Trunc(const Vec0<float> v) {
  const int32_t truncated = static_cast<int32_t>(v.raw);
  if (truncated == 0) return Vec0<float>(v.raw < 0.0f ? -0.0f : 0.0f);
  return Vec0<float>(truncated);
}
HWY_INLINE Vec0<double> Trunc(const Vec0<double> v) {
  const int64_t truncated = static_cast<int64_t>(v.raw);
  if (truncated == 0) return Vec0<double>(v.raw < 0.0 ? -0.0 : 0.0);
  return Vec0<double>(truncated);
}
...
HWY_INLINE Vec0<ToT> PromoteTo(Scalar<ToT> /* tag */, Vec0<FromT> from) {
  static_assert(sizeof(ToT) > sizeof(FromT), "Not promoting");
  return Vec0<ToT>(static_cast<ToT>(from.raw));
}

template <typename FromT, typename ToT>
HWY_INLINE Vec0<ToT> DemoteTo(Scalar<ToT> /* tag */, Vec0<FromT> from) {
...
HWY_INLINE Vec0<uint32_t> U32FromU8(const Vec0<uint8_t> v) {
  return PromoteTo(Scalar<uint32_t>(), v);
}

HWY_INLINE Vec0<uint8_t> U8FromU32(const Vec0<uint32_t> v) {
  return DemoteTo(Scalar<uint8_t>(), v);
}
...
template <typename T>
HWY_API Vec0<T> LowerHalf(Vec0<T> v) {
  return v;
}
...
HWY_INLINE Vec0<uint16_t> ZipLower(const Vec0<uint8_t> a,
                                   const Vec0<uint8_t> b) {
  return Vec0<uint16_t>(static_cast<uint16_t>((uint32_t(b.raw) << 8) + a.raw));
}
HWY_INLINE Vec0<uint32_t> ZipLower(const Vec0<uint16_t> a,
                                   const Vec0<uint16_t> b) {
  return Vec0<uint32_t>((uint32_t(b.raw) << 16) + a.raw);
}
HWY_INLINE Vec0<uint64_t> ZipLower(const Vec0<uint32_t> a,
                                   const Vec0<uint32_t> b) {
  return Vec0<uint64_t>((uint64_t(b.raw) << 32) + a.raw);
}
HWY_INLINE Vec0<int16_t> ZipLower(const Vec0<int8_t> a, const Vec0<int8_t> b) {
  return Vec0<int16_t>(static_cast<int16_t>((int32_t(b.raw) << 8) + a.raw));
}
HWY_INLINE Vec0<int32_t> ZipLower(const Vec0<int16_t> a,
                                  const Vec0<int16_t> b) {
  return Vec0<int32_t>((int32_t(b.raw) << 16) + a.raw);
}
HWY_INLINE Vec0<int64_t> ZipLower(const Vec0<int32_t> a,
                                  const Vec0<int32_t> b) {
  return Vec0<int64_t>((int64_t(b.raw) << 32) + a.raw);
}
