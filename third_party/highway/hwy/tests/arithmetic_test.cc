...
#if !(HWY_CAPS & HWY_CAP_VARIABLE_SHIFT) || HWY_IDE
...
struct TestSignedShifts {
  ...
  HWY_NOINLINE HWY_ATTR void operator()(T /*unused*/, D d) {
    ...
    for (int i = 0; i < static_cast<int>(d.N); ++i) {
...
#if HWY_CAPS & HWY_CAP_VARIABLE_SHIFT
...
struct TestSignedMul {
  ...
  HWY_NOINLINE HWY_ATTR void operator()(T /*unused*/, D d) {
    ...
    for (int i = 0; i < static_cast<int>(d.N); ++i) {
      lanes[i] = static_cast<T>((-T(d.N) + i) * (1 + i));
    }
...
struct TestMulEvenT {
  ...
  HWY_NOINLINE HWY_ATTR void operator()(T /*unused*/, D d) {
    if (d.N == 1) return;
    const HWY_CAPPED(Wide, (d.N + 1) / 2) d2;
    const auto v0 = Zero(d);
...
struct TestSumsOfU8T {
  template <typename T, class D>
  HWY_NOINLINE HWY_ATTR void operator()(T /*unused*/, D d) {
#if HWY_TARGET != HWY_SCALAR && (HWY_CAPS & HWY_CAP_INT64)
    const HWY_CAPPED(uint8_t, d.N * sizeof(uint64_t)) du8;
...
HWY_NOINLINE HWY_ATTR void TestSumsOfU8() {
#if HWY_CAPS & HWY_CAP_INT64
...
HWY_NOINLINE HWY_ATTR void TestSumOfLanes() {
...
#if HWY_ONCE
HWY_EXPORT(TestArithmetic)
#endif

}  // namespace hwy

#if HWY_ONCE
TEST(HwyArithmeticTest, Run) { hwy::RunTest(&hwy::ChooseTestArithmetic); }
#endif
