...
class ThreadParallelRunner {
 public:
  ...
  void RunOnEachThread(const Func& func) {
    ...
    data_func_ =
        reinterpret_cast<JpegxlParallelRunFunction>(&CallClosure<Func>);
 ...
 private:
  ...
  // signature must match JpegxlParallelRunFunction, hence a void* argument.
  ...
  JpegxlParallelRunFunction data_func_;
  void* jpegxl_opaque_;
